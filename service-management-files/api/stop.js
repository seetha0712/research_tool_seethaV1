...
export default async function handler(req, res) {
  ...
  try {
    const endpoint = service.type === 'postgres'
      ? `/postgres/${service.id}/suspend`
      : `/services/${service.id}/suspend`;

    await renderApiCall(endpoint, 'POST');

    // Send email notification (non-blocking)
    sendNotification('stop', service.name, true).catch(console.error);
    ...
    return res.status(200).json({
      ...
      name: service.name,
      action: 'stop',
      message: `Service "${service.name}" has been stopped`,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    // Send failure notification (non-blocking)
    sendNotification('stop', service.name, false, error.message).catch(console.error);
